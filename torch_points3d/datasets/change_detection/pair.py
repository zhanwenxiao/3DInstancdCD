...
class PairBatch(Pair):
    ...
    def from_data_list(data_list):
        ...
        pair.pos_b = np.array([data_s.sample_ori_pos for data_s in data_list_s])
        pair.pos_b_target = np.array([data_t.sample_ori_pos for data_t in data_list_t])
        pair.full_pos = np.array([data_s.full_pos for data_s in data_list_s])
        pair.full_pos_target = np.array([data_t.full_pos for data_t in data_list_t])

        pair.full_rgb = [data_s.full_rgb for data_s in data_list_s]
        pair.full_rgb_target = [data_t.full_rgb for data_t in data_list_t]

        pair.full_change_y = np.array([data_s.full_change_y for data_s in data_list_s])
        pair.full_change_y_target = np.array([data_t.full_change_y for data_t in data_list_t])
        pair.full_inst_y = np.array([data_s.full_inst_y for data_s in data_list_s])
        pair.full_inst_y_target = np.array([data_t.full_inst_y for data_t in data_list_t])
