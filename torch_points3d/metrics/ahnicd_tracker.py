...
#     return instances
#
# class ahniCDTracker(iCDTracker):
#     def __init__(self, dataset, stage="train", wandb_log=False, use_tensorboard: bool = False,
#                  ignore_label: int = IGNORE_LABEL, full_pc: bool = False, full_res: bool = False):
#         super(ahniCDTracker, self).__init__(dataset, stage, wandb_log, use_tensorboard, ignore_label)
...
#
#     def inst_metrics(self, dataset: str = "ahncd"):
#         global CLASS_LABELS
...
class ahniCDTracker(iCDTracker):
    ...
    def track(self, model: model_interface.TrackerInterface, data=None, full_pc=False, conv_classes=None,**kwargs):
        ...
        inputs.change_pred = change_output0
        inputs.change_target_pred = change_output1
        # inputs.mask_pred = inst_output0
        # inputs.mask_pred_target = inst_output1
        # inputs.sem_pred = pred_logits0
        # inputs.sem_pred_target = pred_logits1
        data_l = inputs.to_data_list()
        num_change_pred = self._change_classes
        self.change_metrics(model, data_l, num_change_pred)
    ...
    def get_inst_metric(self, change_pred_dict_0, change_pred_dict_1, change_gt_dict_0, change_gt_dict_1):
        ...
        if length_pred != 0 and length_gt != 0:
            for c_gt_0, c_pred_0, c_gt_1, c_pred_1 in zip(change_gt_dict_0["all_ap"], change_pred_dict_0["all_ap"],
                                                          change_gt_dict_1["all_ap"], change_pred_dict_1["all_ap"]):
        ...
        if length_pred50 != 0 and length_gt50 != 0:
            for c_gt_0, c_pred_0, c_gt_1, c_pred_1 in zip(change_gt_dict_0["all_ap_50%"],
                                                          change_pred_dict_0["all_ap_50%"],
                                                          change_gt_dict_1["all_ap_50%"],
                                                          change_pred_dict_1["all_ap_50%"]):
        ...
        if length_pred25 != 0 and length_gt25 != 0:
            for c_gt_0, c_pred_0, c_gt_1, c_pred_1 in zip(change_gt_dict_0["all_ap_25%"],
                                                          change_pred_dict_0["all_ap_25%"],
                                                          change_gt_dict_1["all_ap_25%"],
                                                          change_pred_dict_1["all_ap_25%"]):
            ...
            for i, area in enumerate(self._areas):
                if area is not None:
                    ...
                    if (self._stage == 'test' or self._stage == 'val') and save_pc:
                        ...
                        # self._dataset.to_inst_ply(pos, pred,
                        #                      os.path.join(saving_path, "pointCloud" +
                        #                                   os.path.dirname(self._ds.filesPC0[i]).split('/')[
                        #                                       -1] + "_inst.ply"),
                        #                      )
                        # self._dataset.to_inst_ply(pos_target, pred_target,
                        #                      os.path.join(saving_path, "pointCloud" +
                        #                                   os.path.dirname(self._ds.filesPC1[i]).split('/')[
                        #                                       -1] + "_inst.ply"),
                        #                      )
