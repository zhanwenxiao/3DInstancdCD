...
class iCDTracker(BaseTracker):
    ...
    def track(self, model: model_interface.TrackerInterface, data=None, conv_classes=None, **kwargs):
        ...
        # if len(outputs) > 3:
        #     change_outputs0, change_outputs1, mask_outputs0, mask_outputs1 = outputs
        #     change_targets0, change_targets1, mask_targets0, mask_targets1, pos_0, pos_1, inst_targets0, inst_targets1 = targets
        #     self._compute_change_metrics_list([change_outputs0, change_outputs1], [change_targets0, change_targets1], conv_classes)
        #     self._compute_inst_metrics_list(change_outputs0, change_targets0, mask_outputs0, mask_targets0, pos_0, inst_targets0)
        #     self._compute_inst_metrics_list(change_outputs1, change_targets1, mask_outputs1, mask_targets1, pos_1, inst_targets1)
        # else:
        #     change_outputs, mask_outputs0, mask_outputs1 = outputs
        #     change_targets, mask_targets0, mask_targets1, pos_0, pos_1, inst_targets0, inst_targets1 = targets
        #     self._compute_change_metrics(change_outputs, change_targets, conv_classes)
        #     self._compute_inst_metrics_list(mask_outputs0, mask_targets0, pos_0, inst_targets0)
        #     self._compute_inst_metrics_list(change_outputs, change_targets, mask_outputs1, mask_targets1, pos_1, inst_targets1)
        ...
        #
        # ########## INST TRACK ##########
        # self._compute_inst_metrics(coord_0, inst_output0, logits_output0, inst_label0, sem_label0, ori_tgt0, batch)
        # self._compute_inst_metrics(coord_1, inst_output1, logits_output1, inst_label1, sem_label1, ori_tgt1, batch_target)
        ########## INST TRACK ##########
    ...
    def _compute_inst_metrics_list(self, c_output, c_target, output, target_full_res, inst_targets, full_target_y,
        ...
        for bid in range(len(prediction[decoder_id]["pred_masks"])):
            ...
            #     knn_idx = knn(_pos, _full_pos, 1)
            #     masks = masks[knn_idx[1, :], :].numpy()
            #     change = c_output[bid][knn_idx[1, :], :].max(dim=-1)[1].numpy()
            #     change_target = full_change[bid].detach().cpu().numpy()
            # else:
            #     masks = masks.numpy()
            ...
            #     _pos = pos[bid]
            #     _full_pos = full_pos[bid]
            #     knn_idx = knn(_pos, _full_pos, 1).cpu().numpy()
            #     change = torch.argmax(c_output[bid], dim=-1).cpu().numpy()  # change = torch.functional.F.softmax(change[bid], dim=-1).detach().cpu()
            #     change_target = full_change[bid].cpu().numpy()
            #     all_knn_idx.append(knn_idx[1, :])
            # else:
            #     masks = masks.numpy()
            #     change = c_output[bid].detach().cpu().max(dim=-1)[1].numpy()  # change = torch.functional.F.softmax(change[bid], dim=-1).detach().cpu()
            #     change_target = c_target[bid].detach().cpu().numpy()
            ...
            # if self._stage == "test":
            #     masks = knn_interpolate(masks, pos[bid].detach().cpu(), full_pos[bid].detach().cpu(), k=1).numpy()
            #     heatmap = knn_interpolate(heatmap, pos[bid].detach().cpu(), full_pos[bid].detach().cpu(), k=1).numpy()
            #     change = knn_interpolate(c_output[bid].detach().cpu(), pos[bid].detach().cpu(), full_pos[bid].detach().cpu(), k=1).numpy()
            #     change = np.argmax(change, axis=-1)
            #     change_target = full_change[bid].detach().cpu().numpy()
            # else:
            #     masks = masks.numpy()
            #     heatmap = heatmap.numpy()
            ...
            # if self._stage == "test":
            #     change = torch.argmax(c_output[bid].detach().cpu(), dim=-1)  # change = torch.functional.F.softmax(change[bid], dim=-1).detach().cpu()
            #     change_target = full_change[bid].detach().cpu().numpy()
            # else:
            #     masks = masks.numpy()
            #     change = c_output[bid].detach().cpu().max(dim=-1)[1].numpy()  # change = torch.functional.F.softmax(change[bid], dim=-1).detach().cpu()
            ...
            #     _full_pos = full_pos[bid]
            #     knn_idx = knn(_pos, _full_pos, 1)
            #     masks = masks[knn_idx[1, :]].detach().cpu().numpy().astype(np.int8)#.astype(np.float16)  # full res
            #     change = c_output[bid][knn_idx[1, :], :].max(dim=-1)[1].cpu().numpy().astype(np.int8)
            #     change_target = full_change[bid].cpu().numpy().astype(np.int8)
            # else:
            #     masks = masks.numpy()
            #     change = c_output[bid].detach().cpu().max(dim=-1)[1].numpy()  # change = torch.functional.F.softmax(change[bid], dim=-1).detach().cpu()
            #     change_target = c_target[bid].detach().cpu().numpy()
