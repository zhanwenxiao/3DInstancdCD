...
class SiamEncFusionSkipKPConv(UnwrappedUnetBasedModel):
    ...
    def set_input(self, data, device):
        ...
        if getattr(data, "pos_target", None) is not None:
            ...
            self.change_labels = data.change_y_target.to(device)
            self.labels = [self.change_labels, self.target_y, self.target_y_target]
        else:
            self.input = data
            self.target_y = None
            self.target_y_target = None
            self.change_labels = None
            self.labels = None
    ...
    def compute_loss(self, loss_weight):
        ...
        print("All loss: {} Change loss: {} Mask loss: {}".format(loss_all, self.loss_seg, mask_loss))
        self.loss += loss_all
        # self.loss += mask_loss
